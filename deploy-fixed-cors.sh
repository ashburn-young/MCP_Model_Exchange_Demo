...
# Configuration
RESOURCE_GROUP_NAME="rg-smart-retail-app7"  # New resource group to prevent conflicts
LOCATION="swedencentral"
ENVIRONMENT_NAME="retailassistant10"  # New environment name to prevent conflicts
...
az containerapp create \
  ...
  --min-replicas 1 \
  --max-replicas 3 \
  --env-vars AZURE_OPENAI_API_KEY=$AZURE_OPENAI_API_KEY AZURE_OPENAI_ENDPOINT=$AZURE_OPENAI_ENDPOINT AZURE_OPENAI_MODEL_GROUP=$AZURE_OPENAI_MODEL_GROUP PORT=3002 \
  --output table
...
# Update Container App CORS settings to allow all origins during development
echo -e "${YELLOW}▶ STEP 7: Updating CORS settings in Container App...${NC}"
...
echo -e "${GREEN}API & Web App URL: https://$CONTAINER_APP_URL${NC}"
